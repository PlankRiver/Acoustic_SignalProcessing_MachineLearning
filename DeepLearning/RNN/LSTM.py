...
class LSTMModel(nn.Module):
    ...
    def forward(self, x, state):
        # x: (Batch, Seq_len)
        # state: 这里是一个元组 (H, C)
        #   H: (Num_layers, Batch, Hidden_size) - 隐状态
        #   C: (Num_layers, Batch, Hidden_size) - 细胞状态(记忆)

        # 1. Embedding
        X = self.embedding(x)

        # 2. LSTM Forward
        # Y: 输出特征 (Batch, Seq_len, Hidden_size)
        # state: 更新后的 (H_n, C_n) 元组
...
EMBED_SIZE = 128
HIDDEN_SIZE = 256
LR = 0.001  # LSTM 参数更多，有时需要调整 LR，这里保持一致
EPOCHS = 200
...
# 5. 训练循环
# ==========================================
print(f"Start Training LSTM on Time Machine Dataset...")
...
for epoch in range(EPOCHS):
    ...
    for X, Y in train_loader:
        ...
        # 【修改点 C】: 截断梯度流 (Detach)
        # 因为 LSTM 的 state 是一个 tuple (h, c)，所以要分别 detach
...
# 6. 保存模型
# ==========================================
model_path = "lstm_time_machine.pth"
checkpoint = {
